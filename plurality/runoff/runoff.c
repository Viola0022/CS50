...
void tabulate(void)
{
    for (int i = 0; i < voter_count; i++)
    {
        ...
        {
            // If top is eliminated, move to next
            top_preference++;
        }
        // Increment vote count for top candidate
...
int find_min(void)
{
    int min_votes = voter_count; // min_votes to the maximum possible value
...
// Return true if the election is same between all
bool is_tie(int min)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        {
            return false; // At least one candidate has a different vote count
        }
    }

    return true; // All non-eliminated candidates have the same vote count
}

// Eliminate the candidate in last place
void eliminate(int min)
{
    // eliminate cnadidates with minimum vote count
    for (int i = 0; i < candidate_count; i++)
    {
        if (!candidates[i].eliminated && candidates[i].votes == min)
        {
